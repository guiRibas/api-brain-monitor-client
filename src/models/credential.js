...
class Credential {
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        let query = connection.format(queryUpdate,
          ['credential', 'api_password', passHash, 'login', this.login]);
  ...
  authenticate() {
    ...
    let query = connection.format(queryFindApiPassword,
      ['id', 'api_password', 'credential', 'login', this.login]);
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        if (!result[0][0]) throw new Error(
          'Erro. Credencial informada não existe na base de dados!'
        )
